 ...
 */
var xmlHttp;
function verify_releaseInterest() {
    //使用dom的方式获取文本框中的值
    ...
    xmlHttp.onreadystatechange = callback_releaseInterest;
    //3.设置连接信息
    ...
    //post方式发送数据
    xmlHttp.send("action=release&userName=" + userName + "&content=" + content);
    //4.发送数据，开始和服务器端进行交互
...
//回调函数
function callback_releaseInterest() {
    //5.接收响应数据
    //判断对象的状态 是交互完成
        ...
        if (xmlHttp.status == 200) {
            ...
            if (messageNodes.length > 0) {
                ...
                if (responseMessage == 'success') {
                    //var userName = document.getElementById("userName").innerText;
                    //window.location.href = "../view/interest.php?userName=" + userName;
