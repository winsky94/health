...
function get_page_num() {
    ...
    if (xmlHttp != null) {
        ...
        xmlHttp.open("POST", "../controller/momentHandle.php");
        //post方式需要自己设置http的请求头
        xmlHttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        //post方式发送数据
        xmlHttp.send("action=getMomentsPageNum");
...
function get_moments(current_page) {
    var xmlhttp = getXmlHttp();
    if (xmlhttp != null) {
        xmlhttp.open('POST', '../controller/momentHandle.php');
        xmlhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xmlhttp.onreadystatechange = function () {
            onMomentsResponse(xmlhttp)
        };
        xmlhttp.send("action=getMoments&pageNum=" + current_page);
    } else {
        alert('Your browser does not support XMLHttpRequest.');
    }
}

function onMomentsResponse(xmlhttp) {
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        //file_elements是一个数组
        ...
        var file_elements = xmlDoc.getElementsByTagName("moment");
        var txt = write_moment_list(file_elements);
        $("#moments_list").html(txt);
    }
}
